...
└─┬──┬──┬───┘
  │  │  │
VCC GND AOUT (또는 AO, DO 중 AO만 사용)
```

⚠️ **중요:** 토양센서에는 보통 3~4개 핀이 있습니다:
- **VCC**: 전원 (+) 5V
- **GND**: 접지 (-)
- **AOUT (또는 AO)**: 아날로그 출력 (이것만 사용!)
- **DOUT (또는 DO)**: 디지털 출력 (사용 안 함)
...
4. 각 센서의 VCC를 (+) 레일에 연결
5. 각 센서의 GND를 (-) 레일에 연결
6. **센서1의 AOUT(AO) → A0**, **센서2의 AOUT(AO) → A1**
7. DO 핀은 사용하지 않으므로 **연결하지 마세요!**

**확인 포인트:**
- [ ] 센서 전원 LED가 켜지는지 확인
- [ ] **AOUT(AO) 핀을 A0/A1에 연결했는지** 재확인
- [ ] DO 핀은 비어있는지 확인
- [ ] 핀이 느슨하지 않은지 확인
